...
for i, (res, config, pred, actual) in enumerate(sorted_combined, start=1):
    ...
    plt.title(f"LSTM - Funkcja straty MSE (Model {i})")  
    plt.xlabel("Epoka")
    plt.ylabel("Strata (MSE)")
    plt.legend(loc='upper right')
    plt.grid(True, linestyle='--', linewidth=0.5, alpha=0.5)
    plt.tight_layout()
    plt.savefig(f"Plots/{index_name}/loss/loss_model_{i}_LSTM_{filename_suffix}.png")  
    plt.close()
    ...
    plt.plot(last_dates, pred, label='Wartości prognozowane (LSTM)', color='darkred', linewidth=2) 
    plt.title(f"LSTM - Prognoza ceny zamknięcia (Model {i})")  
    plt.xlabel("Data")
