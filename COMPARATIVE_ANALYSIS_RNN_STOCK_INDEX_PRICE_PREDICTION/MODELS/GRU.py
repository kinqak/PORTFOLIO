...
# Zapis wyników
pd.DataFrame(results).to_excel(f"Results_GRU_{index_name}.xlsx", index=False)

os.makedirs(f"Plots/{index_name}_GRU/loss", exist_ok=True)
os.makedirs(f"Plots/{index_name}_GRU/forecast", exist_ok=True)
...
for i, (res, config, pred, actual) in enumerate(sorted_combined, start=1):
    ...
    plt.title(f"GRU - Funkcja straty MSE (Model {i})")  
    plt.xlabel("Epoka")
    plt.ylabel("Strata (MSE)")
    plt.legend(loc='upper right')
    plt.grid(True, linestyle='--', linewidth=0.5, alpha=0.5)
    plt.tight_layout()
    plt.savefig(f"Plots/{index_name}_GRU/loss/loss_model_{i}_GRU_{filename_suffix}.png")  
    plt.close()
    ...
    plt.plot(last_dates, pred, label='Wartości prognozowane (GRU)', color='darkred', linewidth=2) 
    plt.title(f"GRU - Prognoza ceny zamknięcia (Model {i})")  
    plt.xlabel("Data")
    ...
    plt.tight_layout()
    plt.savefig(f"Plots/{index_name}_GRU/forecast/forecast_model_{i}_GRU_{filename_suffix}.png")  
    plt.close()